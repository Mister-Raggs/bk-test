...
def test_input_document_init_local_path(mocker):
    ...
    validation_successful_local_file_path = (
        "C:\\Users\\saiki\\OneDrive\\Documents\\Citadel-IDP-Src\\local-blob-storage\\Company-A\\Validation-Successful"
    )
    ...
    assert (
        input_document.validation_successful_document_url
        == pathlib.Path(validation_successful_local_file_path).as_uri()
    )
...
def test_input_document_init_both_paths_raises_exception(mocker):
    validation_successful_local_file_path = (
        "C:\\Users\\saiki\\OneDrive\\Documents\\Citadel-IDP-Src\\local-blob-storage\\Company-B\\Validation-Successful"
    )
    form_recognizer_endpoint = "https://aarkformrecognizer.cognitiveservices.azure.com/"
    document_type = "Invoice4"
    with pytest.raises(CitadelIDPBackendException):
